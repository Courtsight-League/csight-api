...
const resolveSiteUrl = (req: HandlerRequest, configuredSiteUrl?: string) => {
...
const readJsonBody = async (req: HandlerRequest) => {
...
export const createAdminUsersHandler = (env: AdminUserHandlerEnv) => {
  const supabaseUrl = String(env.supabaseUrl || '').trim();
  const serviceKey = String(env.serviceKey || '').trim();
  const siteUrl = String(env.siteUrl || '').trim();
  const supabaseAdmin =
